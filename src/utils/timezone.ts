...
export function getTimezoneWithZone(zone: string) {
  for (const tz of timezones) {
    if (tz.utc.indexOf(zone) !== -1) return tz;
  }
}

export function getTimezoneWithValue(value: string) {
  for (const tz of timezones) {
    console.log(tz, tz.value);
    if (tz.value === value) return tz;
  }
}

export function getTimezoneUTCList() {
  let utc: {
    value: string;
    offset: number;
  }[] = [];
